...
bool read(InStream &stream) {
  ...
  for(int i = 1; i < 2*N; i++) {
    S[i] = string(i<N?(N-i-1):(i-N), ' ') + stream.readLine();
  }

  /* 1. map이 유효한가? */
  for(int i = 0; i < 2*N; i++) {
    if(S[i].size() != D[i].size())
      stream.quitf(_wa, "");

    // 2. 빈 칸을 제대로 채웠는지 확인
    for(int j = 0; j < D[i].size(); j++) {
      if(D[i][j] == ' ') {
        if(S[i][j] != ' ') stream.quitf(_fail, "");
      }else if(D[i][j] == '.') {
        if(S[i][j] != '.') stream.quitf(_wa, "");
      }else if(D[i][j] == '0') {
        if('1' <= S[i][j] && S[i][j] <= '6') {
        }else {
          stream.quitf(_wa, "");
  ...
  vector< vector<int> > G[20]; int num_groups = 0;
  for(int i = 0; i < 2*N; i++) {
    G[i].resize(S[i].size());
  }

  // 맵은 얼추 맞으니 이제 coloring이 제대로 되었는지 확인.
  for(int i = 0; i < 2*N; i++) {
    int o = i<N?(N-i-1):(i-N);
    for(int j = 0; j < S[i].size(); j++) {
      // (i, j) 칸이 '가운데' 칸인지 확인할 것.

      // 케이스 1: ㅡ
      if(j-1 >= 0 && j+1 < S[i].size() && D[i].substr(j-1, 3) == "000") {
        // 가운데 맞으니까 일단 같은 색인지 확인
        if(S[i][j-1] == S[i][j] && S[i][j] == S[i][j+1]) {
          // 같은 색이라면 한 그룹으로 색칠되었다는 것이므로, 그룹에 추가하자.
          num_groups += 1;
          G[i][j-1].push_back(num_groups);
          G[i][j  ].push_back(num_groups);
          G[i][j+1].push_back(num_groups);
        }
      }

      // 케이스 2: ㄱ
      int dx = ((j - o) % 2 == 0) ^ (i >= N) ? +1 : -1;
      for(int dy = -1; dy <= 1; dy += 2) {
        int i1 = i+dx, j1 = j;
        int i2 = i, j2 = j+dy;
        if(0 <= i1 && i1 < 2*N && 0 <= j1 && j1 < S[i1].size() &&
           0 <= i2 && i2 < 2*N && 0 <= j2 && j2 < S[i2].size()) {
          if(D[i][j] == '0' && D[i1][j1] == '0' && D[i2][j2] == '0' &&
              S[i][j] == S[i1][j1] && S[i1][j1] == S[i2][j2]) {
            num_groups += 1;
            G[i ][j ].push_back(num_groups);
            G[i1][j1].push_back(num_groups);
            G[i2][j2].push_back(num_groups);
          }
        }
      }
...
int main(int argc, char * argv[]) {
  ...
  for(int i = 0; i < 2*N; i++) {
    D[i] = inf.readToken();
    ensuref(D[i].size() == (i < N) ? 2*(N+i)+1 : 2*(3*N-1-i)+1, "map size of line %d is wrong", i+1);
    D[i] = string(i<N?(N-i-1):(i-N), ' ') + D[i];
  }
    ...
    quit(_fail, "Judge couldn't find solution, but contestant could.");
  }else if(ja && !pa) {
    quit(_wa, "");
  }
    return 0;
