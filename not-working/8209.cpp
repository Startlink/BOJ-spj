...
int main(int argc, char **argv){
  ...
  oi::Scanner out(argv[3], own_error_function, oi::PL);
  oi::Scanner out_correct(argv[2], own_error_function, oi::PL);
  
  out_correct.readString(answer, 20);
  if(strcmp(answer, nie) == 0){
    out.readString(answer, 20);
    out.readEofOrEoln();
    if(strcmp(answer, nie) == 0) return 0;
    else assert(0);
  }
  ...
  REP(i, nk){
	...
	S2.erase(MP(x,y));
      ...
      exist.clear();
      if(exist.find(y) != exist.end()) assert(0);
      else
        exist.insert(y);
      x = y;
    }
