...
int main(int argc, char * argv[])
{
	...
	for(int t = 1; t <= T; t++){
		...
		for(int i = 0; i < M; i++){
			...
			if( a != A || b != B ) quitf(_wa, "edge order mismatch");
			deg[a]++; deg[b]++;
			G[pii(a, b)] = c;
		}
		int mx = 0;
		...
		for(auto e : G){
			...
			if( c <= 0 || c > mx ) quitf(_wa, "%d : color out of range", c);
			used[a].insert(c), used[b].insert(c);
		}
		for(int i = 1; i <= N; i++) if( used[i].size() != deg[i] ) quitf(_wa, "%d : Vertex something wrong, %d %d %d", t, i, used[i].size(), deg[i]);
	}
	quitf(_ok, "correct");
	return 0;
}
